        ...
        {isLoading ? <div className="p-12 text-center text-gray-800">Loading...</div> : drivers.length === 0 ? (
          ...
          <table className="data-table">
            ...
            <tbody>
              {drivers.map((d) => (
                <tr key={d._id}>
                  <td><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center text-gray-900 text-xs font-bold">{d.user?.name?.charAt(0)}</div><span className="text-gray-900">{d.user?.name}</span></div></td>
                  <td className="text-gray-700 text-xs">{d.user?.phone}</td>
                  ...
                  <td className="text-gray-900 font-medium">{d.completedDeliveries}</td>
