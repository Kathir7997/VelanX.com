  ...
  return (
    <div>
      ...
      <div className="glass-card overflow-hidden">
        {isLoading ? <div className="p-12 text-center text-gray-800">Loading...</div> : (data || []).length === 0 ? (
          <div className="p-12 text-center"><Warehouse size={40} className="text-gray-600 mx-auto mb-3" /><p className="text-gray-700">No warehouses found</p></div>
        ) : (
            ...
            <tbody>
              {(data || []).map((w) => (
                <tr key={w._id}>
                  ...
                  <td>{w.capacity}</td>
                  <td><div className="flex items-center gap-2"><div className="w-16 h-1.5 bg-black/5 rounded-full"><div className="h-full bg-primary-500 rounded-full" style={{ width: `${Math.min(100, (w.currentLoad / w.capacity) * 100)}%` }} /></div><span className="text-xs text-gray-700">{w.currentLoad}</span></div></td>
