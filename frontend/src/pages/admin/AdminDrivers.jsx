        ...
        {isLoading ? <div className="p-12 text-center text-gray-500">Loading...</div> : drivers.length === 0 ? (
          ...
          <table className="data-table">
            ...
            <tbody>
              {drivers.map((d) => (
                <tr key={d._id}>
                  <td><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center text-white text-xs font-bold">{d.user?.name?.charAt(0)}</div><div><p className="text-white text-sm">{d.user?.name}</p><p className="text-gray-500 text-xs">{d.user?.phone}</p></div></div></td>
                  ...
                  <td className="text-white font-semibold">{d.completedDeliveries}</td>
                  <td className="text-gray-400 text-xs">{d.assignedVehicle?.vehicleNumber || '—'}</td>
                  <td>
                    <button id={`delete-driver-${d._id}`} onClick={() => { if (confirm('Remove driver?')) deleteMutation.mutate(d._id); }} className="p-1.5 rounded-lg hover:bg-red-500/10 text-gray-400 hover:text-red-400 transition-all">
                      <Trash2 size={14} />
                    </button>
                  </td>
                </tr>
